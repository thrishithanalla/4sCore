"""
Error Master Repository
Handles all database operations for the error_master collection.
"""
...
class ErrorMasterRepository(BaseRepository):
    """Repository class for error master CRUD operations"""

    OBJECTID_FIELDS = ["moduleId", "createdBy", "updatedBy", "deletedBy"]

    def __init__(self, db: AsyncIOMotorDatabase = None):
        super().__init__(Collections.ERROR_MASTER, db)
    ...
    async def find_by_module(self, module_id: str, include_deleted: bool = False) -> List[dict]:
        """Find all error masters for a module"""
    ...
    async def exists_by_code(self, code: str, exclude_id: str = None) -> bool:
        """Check if error master with code exists"""
        return await self.exists_by_field("code", code, exclude_id)

    async def get_all_active(self) -> List[dict]:
        """Get all non-deleted error masters"""
