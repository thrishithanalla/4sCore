"""
Prompt Repository
Handles all database operations for the prompt_master collection.
"""
...
class PromptRepository(BaseRepository):
    """Repository class for prompt CRUD operations"""

    OBJECTID_FIELDS = ["moduleId", "createdBy", "updatedBy", "deletedBy"]

    def __init__(self, db: AsyncIOMotorDatabase = None):
        super().__init__(Collections.PROMPT_MASTER, db)
    ...
    async def find_all(
        ...
        """Find all prompts with pagination"""
    ...
    async def find_active_prompts(self) -> List[dict]:
        """Find all active (enabled) prompts"""
        docs, _ = await self.find_all({"isDelete": False, "isActive": True})
        return docs
